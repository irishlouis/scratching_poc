...
dat <- BreastCancer[complete.cases(BreastCancer), -1]  # remove the ID column and incomplete rows
dat <- cbind(as.data.frame(apply(dat[,names(dat) != "Class"], 2, function(x) as.numeric(as.character(x)))), Class = dat$Class)
str(dat)
dat_h2o <- as.h2o(destination_frame = "localH2O", x = dat)
...
for (n in 1:n_run) {
  ...
  model <- h2o.deeplearning(x = 1:9,  # column numbers for predictors
                            ...
                            training_frame = dat_h2o[sort(row_train), ],
                            activation = "Tanh",
  ...
  yhat_train <- h2o.predict(model, dat_h2o[sort(row_train), ])$predict
  yhat_train <- as.factor(as.matrix(yhat_train))
  yhat_test <- h2o.predict(model, dat_h2o[sort(row_test), ])$predict
